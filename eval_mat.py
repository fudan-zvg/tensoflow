...
class MaterialTester:
    ...
    def _init_dataset(self): 
        ...
        # self.dataloader = DataLoader(self.test_ids, 1, False, num_workers=self.cfg['worker_num'])  # cause error
        self.dataloader = DataLoader(self.test_ids, 1, False, num_workers=0)
    ...
    def _init_network(self):
        ...
        if os.path.exists(self.pth_fn):
            ...
            self.network = name2renderer[self.cfg['network']](self.cfg, training=False).eval().cuda()
    ...
    def relight(self):
        if self.database_type == 'tensoSDF':
            for env_name in self.env_maps:
                ...
                relight_save_dir = f'data/relight/{self.model_name}/{save_dir_name}'
                cmds=[
            ...
            relight_save_dir = f'data/relight/orb/{self.model_name}/noScale/{save_dir_name}'
    ...
    def eval_relight_results(self):
        ...
        for env_name in self.env_maps:
            relight_dir = f'data/relight/{self.model_name}/{self.scene_name}_{env_name}'
        ...
        with open(f'data/relight/{self.model_name}/{self.scene_name}_relight_record.txt', 'a') as f:
            f.write(msg)        
...
if __name__ == '__main__':
    ...
    parser.add_argument('--blender', type=str, default='/SSD_DISK/users/guchun/blender-3.6.22-linux-x64/blender', 
                        help='the blender path')
    parser.add_argument('--env_dir', type=str, default='data/env_maps/tensoIR/envmaps_1k_exr', 
                        help='env dir')
    #---------------------------Following settings are used in orb datasets-----------------------------------
    parser.add_argument('--orb_relight_gt_dir', type=str, default='nerf_data/orb/ground_truth',
    ...
    parser.add_argument('--orb_blender_dir', type=str, default='nerf_data/orb/blender_LDR',
    ...
    matTester = MaterialTester(cfg, flags.blender, flags.env_dir, orb_settings)
