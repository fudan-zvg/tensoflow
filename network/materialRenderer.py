...
from network.fields import TensoSDF, ShapeShadingNetwork
...
class MaterialRenderer(nn.Module):
    ...
    def __init__(self, cfg, training=True, nvs=False):
        ...
        if os.path.exists(self.cfg['geo_model_path']):
            checkpoint=torch.load(self.cfg['geo_model_path'], weights_only=False)
    ...
    def init_sdf(self, ckpt):
        ...
        self.sdf_network = TensoSDF(
            ...
            sdf_n_comp=configs['sdf_n_comp'], sdf_dim=configs['sdf_dim'], app_dim=configs['app_dim'], sdf_multires=configs.get('sdf_multires', 0)).to(self.device)
    ...
    def _init_dataset(self, training):
        ...
        if training:
            ...
            if self.cfg['downsample_ratio'] != 1:
                self.test_imgs_info = imgs_info_downsample(self.test_imgs_info, self.cfg['downsample_ratio'])
            ...
            if self.cfg['nerfDataType']:
                self.train_batch = self._construct_ray_batch_nerf(self.train_imgs_info)        
            else:    
                self.train_batch = self._construct_ray_batch(self.train_imgs_info, apply_mask=self.cfg['apply_mask_loss'])
            
            self.filtering_train_rays()
            
            # self.train_batch = self._get_trace_ray_batch_info(self.train_batch)
    ...
    def _init_shader(self):
        ...
        self.shader_network = MCShadingNetwork(self.cfg['shader_cfg'], lambda o,d: self.trace(o+2*self.unit_size*d, d), aabb=self.aabb)

    def get_train_opt_params(self, learning_rate_xyz, learning_rate_net, learning_rate_env):
        grad_vars = []
        grad_vars += self.shader_network.get_optparam_groups(learning_rate_xyz, learning_rate_net, learning_rate_env)
        return grad_vars
    ...
    def load_ckpt(self, ckpt):
       self.load_state_dict(ckpt['network_state_dict'], strict=False)
    ...
    def trace_sdf_in_batch(self, rays_o, rays_d, batch_size=10240*5, cpu=False):
        inters, normals, depth, hit_mask = [], [], [], []
        rn = rays_o.shape[0]
        for ri in range(0, rn, batch_size):
    ...
    def trace_sdf_with_mesh(self, rays_o, rays_d, sn0, sn1):
        ...
        if torch.sum(hit_mask) > 0:
            ...
            flip_mask = (normals[hit_mask]*rays_d[hit_mask]).sum(dim=-1) >= 0
            normals_copy = normals[hit_mask]
            normals_copy[flip_mask] *= -1
            normals[hit_mask] = normals_copy
    ...
    @torch.no_grad()
    def filtering_train_rays(self, device='cuda', chunk=512**2):
        print('========> filtering rays ...')
        # path = "filter_train_rays.pt"
        # if os.path.exists(path):
        #     self.train_batch = torch.load(path, weights_only=False)
        #     self.tbn = self.train_batch['rays_o'].shape[0]
        #     return
        ...
        outputs = {}
        N = self.train_batch['rays_o'].shape[0]
        idx_chunks = torch.split(torch.arange(N), chunk)        
        torch.set_default_tensor_type('torch.cuda.FloatTensor')
        for idx_chunk in tqdm(idx_chunks):
            outputs_cur = {}
            for k, v in self.train_batch.items():
                outputs_cur[k] = v[idx_chunk].to(device)
            outputs_cur = self._get_trace_ray_batch_info(outputs_cur)
            
            for k in outputs_cur.keys():
                if k not in outputs:
                    outputs[k] = []
    ...
    def _construct_ray_batch_nerf(self, imgs_info, device='cpu'):
        ...
        ray_batch={
            ...
            'human_poses': human_poses.reshape(-1, 3, 4).to(device),
            'rgb': rgb.reshape(-1, 3).to(device),
        }
        return ray_batch
    ...
    def train_step(self, step):
        ...
        # fetch to gpu
        ray_batch_info = {k: v[self.train_batch_i:self.train_batch_i + rn].cuda() for k, v in self.train_batch.items()}
        ...
        self.shader_network.update_step(step)
        ...
        shade_outputs['psnr'] = 20 * torch.log10(1.0 / torch.sqrt(F.mse_loss(shade_outputs['rgb_pr'], shade_outputs['rgb_gt'])))
        if self.cfg['reg_mat']:
    ...
    def test_step(self, index):
        ...
        output_keys = {'rgb_gt':3, 'rgb_pr':3, 'normal': 3, 
        ...
        if self.shader_network.use_nis:
            output_keys.update({'rgb_gt_nis':3, 'rgb_pr_nis':3, 'normal_nis': 3,
                       'specular_light_nis':3, 'specular_color_nis':3, 
                       'diffuse_light_nis':3, 'diffuse_color_nis':3, 
                       'albedo_nis':3, 'metallic_nis':1, 'roughness_nis':1,
                       'occ_trace_nis':1, 'indirect_light_nis':3})
        ...
        for ri in range(0, rn, trn):
            ...
            if torch.sum(hit_mask)>0:
                ...
                outputs_cur['normal'][hit_mask] = shade_outputs['normal']
                ...
                if self.shader_network.use_nis:
                    outputs_cur['rgb_pr_nis'][hit_mask] = shade_outputs['rgb_pr_nis']
                    outputs_cur['rgb_gt_nis'][hit_mask] = rgb_gt
                    outputs_cur['specular_light_nis'][hit_mask] = shade_outputs['specular_light_nis']
                    outputs_cur['specular_color_nis'][hit_mask] = shade_outputs['specular_color_nis']
                    outputs_cur['diffuse_color_nis'][hit_mask] = shade_outputs['diffuse_color_nis']
                    outputs_cur['diffuse_light_nis'][hit_mask] = shade_outputs['diffuse_light_nis']
                    outputs_cur['albedo_nis'][hit_mask] = shade_outputs['albedo_nis']
                    outputs_cur['normal_nis'][hit_mask] = shade_outputs['normal_nis']
                    outputs_cur['metallic_nis'][hit_mask] = shade_outputs['metallic_nis']
                    outputs_cur['roughness_nis'][hit_mask] = torch.sqrt(shade_outputs['roughness_nis']) # note: we assume predictions are roughness squared
                    outputs_cur['occ_trace_nis'][hit_mask] = shade_outputs['visibility_nis']
                    outputs_cur['indirect_light_nis'][hit_mask] = shade_outputs['indirect_light_nis']
        ...
        outputs['envmap'] = self.shader_network.env_light(512, 1024)
        return outputs
    ...
    def nvs(self, pose, K, h, w):
        ...
        rn = h * w
        trn = 512
        output_keys = {'color':3, 'normal':3, 'spec_light':3, 'diff_light':3, 'indirect_light':3, 'spec_color':3, 'diff_color':3, 'albedo':3, 'roughness':1, 'metallic':1, 'occ_trace':1, 'variance_diffuse_vis': 1, 'variance_specular_vis': 1, 'variance_diffuse_vis_nis': 1, 'variance_specular_vis_nis': 1}
        outputs = {k:[] for k in output_keys.keys()}
        for ri in range(0, rn, trn):
            nvs_ray_batch = {}
            for k, v in ray_batch.items():
            ...
            if torch.sum(hit_mask)>0:
                ...
                outputs_cur['indirect_light'][hit_mask] = shade_outputs['indirect_light']
                outputs_cur['occ_trace'][hit_mask] = shade_outputs['visibility']
                # outputs_cur['variance_diffuse_vis'][hit_mask] = shade_outputs['variance_diffuse_vis']
                # outputs_cur['variance_diffuse_vis_nis'][hit_mask] = shade_outputs['variance_diffuse_vis_nis']
                # outputs_cur['variance_specular_vis'][hit_mask] = shade_outputs['variance_specular_vis']
                # outputs_cur['variance_specular_vis_nis'][hit_mask] = shade_outputs['variance_specular_vis_nis']
    ...
    def forward(self, data):
        ...
        if is_train:
            if self.shader_network.cfg['outer_light_version']=='envlight':
                self.shader_network.outer_light.build_mips_direct()
