...
class NISLoss(Loss):
    ...
    def __call__(self, data_pr, data_gt, step, *args, **kwargs):
        outputs = {}
        if 'loss_nis' in data_pr:
            outputs['loss_nis'] = data_pr['loss_nis'].reshape(1) * self.cfg['nis_loss_weight']
        if 'nis_var' in data_pr:
            outputs['nis_var'] = data_pr['nis_var']
        if 'variance' in data_pr:
            outputs['variance'] = data_pr['variance']
        return outputs
